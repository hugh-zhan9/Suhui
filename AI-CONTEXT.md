...
- **表对象随方言重绑**（关键约束）：`packages/internal/database/src/schemas/index.ts`
  用 `export let` + `onRuntimeDbTypeChange` 实时绑定切换 28 张表。
...
- 正文按段落、标题、列表项、表格单元格、换行和空行分批，普通段落独立请求；超过 2,000 UTF-16 code units 的大段落优先沿句子边界拆分，超长单句沿空白边界或安全字符边界继续拆分。同段行内文本节点放入同批，译文按原节点和片段位置重建。阅读区请求只翻译标题与指定正文，共享最多五个请求槽位，正文无需等待标题完成；列表请求按需翻译标题与摘要。每批完成后通过仅发给调用 renderer 的 IPC 事件更新会话内部分结果；乱序结果按原槽位合并，每批成功先写 `translation_batches` 表再发布进度；当前范围全部成功后写 `translations` 完整字段，并在数据库原子 upsert 中保留同源其他范围的当前字段（2026-09-15 用户要求调整）
- 阅读区与列表通过 `withContent` 分开：`true` 为标题与指定正文，空正文也不转摘要；`false` 或省略为列表标题与摘要。常规生成按文章/语言串行；不同失败批次的手动重试并发执行，同文章/语言最多五个 provider 请求，同批在途去重；后续常规生成等待已受理重试，前一个任务失败不会传递；renderer 独立记录两种范围进度，只合并各自字段，列表请求不清除正文部分结果。源内容版本改变时同时清除两类旧请求状态，拒绝迟到结果。日志 `scope=reader/list` 区分范围；本次范围分离尚未安装。
- 批次失败继续与手动重试沿用 [D-009](docs/loopx/design/2026-09-04-progressive-translation/设计提案.md#D-009)：正文失败处保留原文并显示重试，标题失败在状态区操作；首次运行期间按钮禁用，结束后不同失败批次可同时重试，成功槽位复用。部分失败不得显示完成，所有错误可复制；重试结束后仍等待其他在途请求。快照 revision 防止迟到结果回退。
- 2026-09-16 用户明确要求数据库长期保存，并裁定切换 AI 服务/模型继续使用，只有主动重新翻译才覆盖。`translation_batches` 保存成功标题/正文批次，原文指纹与分批版本校验后复用；无时间过期、不因配置变化删除。打开/重启先显示已存段落，只补缺失批次；数据库保存失败在本会话保留译文，重试只补写。完整译文优先读取，凭据仅在需要新请求时读取一次，失败也保留已恢复的阅读内容。标题及当前正文可通过头部“重新翻译全文”主动重做，其他正文/摘要保留；同时使同文章/语言旧会话失效，避免旧标题写回。主进程只保留最近 16 个会话对象，但淘汰不删除已存批次；失效会话通过“继续翻译”读库恢复。设计见 [D-010](docs/loopx/design/2026-09-04-progressive-translation/设计提案.md#D-010)，代码已验证，尚未安装。
- 双方言新增成功批次表与唯一键，文章删除级联；完整备份写 v2、兼容读取 v1/v2，包含成功批次，数据库转换保留译文。merge 按完整译文 created_at / 批次 updated_at 保留较新数据；replace 使用确认快照。翻译任务纳入 DBManager 维护屏障，转换从导出前暂停新增翻译，等在途保存完成后进行；切库/恢复清内存会话，避免旧结果写回。没有自动重试、超时调整或 Remote 翻译接口。
...
- 只有全局自动、文章按钮或文章 action 启用时才发送标题、摘要或正文；数据库译文按原文版本复用；更换服务配置等待在途生成和所有重试完成，只切换后续缺失批次使用的服务，不清除已有译文
...
- `.suhui-backup` 写 v2、兼容读 v1/v2，使用流式 NDJSON、记录计数与 SHA-256 校验；支持 merge 与带安全快照、二次确认、事务回滚的 replace
