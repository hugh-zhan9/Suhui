...
> 最后更新时间：2026-09-16（批次翻译失败隔离与单批重试；笔记与高亮汇总页；渐进式文章与选区翻译；SQLite 支持与 Postgres ↔ SQLite 双向转换；渲染层远端接口清零）
...
- 2026-09-16 用户要求批次失败继续与单批手动重试：阅读区 provider 批次失败只记录本批，后续标题/正文继续。正文失败处保留原文并显示重试，标题失败在状态区操作；首次运行期间按钮禁用，结束后只重发所选失败批次，成功槽位复用。部分失败不得显示完成；重试或缓存写入错误可复制，并提供显式“重新翻译全文”。main 在原应用服务内最多保留 16 个未完成会话检查点，绑定原文/配置指纹及正文类型；源内容或配置变化、重启或淘汰后拒绝旧重试，不自动从头请求。仅全部成功写完整缓存；列表/选区仍保持原失败语义。无自动重试、超时调整、迁移或 Remote 接口。设计见 [批次恢复 D-009](docs/loopx/design/2026-09-04-progressive-translation/设计提案.md#D-009)，尚未安装到本机应用。
