...
## 五路并发与持久保存（2026-09-16）

正文/标题共用每文章语言 5 个槽位，不同失败批次可同时重试；同批请求去重，第 6 个等待。成功批次写入 `translation_batches` 后发布进度，完整字段仍在 `translations`。日志沿用 `cache.*` 事件名称表示数据库读写，不代表有 TTL；换模型不清除记录。`batch.persist_failed` 表示生成成功但数据库保存失败，本会话手动重试会补保存、不再调用 AI。

普通打开会读取已存批次并补缺失；头部“重新翻译全文”才清除当前标题/正文结果后重新请求。配置解密失败为一次任务错误，已恢复的部分译文仍显示；备份恢复或切库期间暂停翻译，待已受理任务落库后进行。
