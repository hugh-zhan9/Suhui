...
- 脚本会自动关闭旧应用、执行 `pnpm --filter suhui build:electron-vite` 与 `FOLO_NO_SIGN=1 node scripts/run-electron-forge.mjs package`、直接安装打包产出的 `.app`、重新做一次本地 ad-hoc 签名、清理 quarantine，并直接启动应用。
