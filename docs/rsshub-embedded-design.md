...
- 不保证"全部 1000+ 路由"首版均可用，仅内置 Top 30
- 不实现远程插件动态安装路由
- 不引入云端同步或远端配置中心
...
**核心机制**：
- 使用 `child_process.fork()` 启动 RSSHub，**复用 Electron 内置 Node.js 运行时，无需目标机器安装 Node**
...
构建时通过**路由白名单**控制打包哪些路由，其余路由在用户需要时提示"该路由需要自建实例"。
