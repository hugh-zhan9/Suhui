...
- 条目详情页直接补作者和原文链接的收益很高，因为远程端阅读往往需要快速判断来源并跳转回原站，不需要等完整设置页或更复杂动作就能显著提升可用性。
- “读完自动前进”更适合抽成纯函数单测，而不是把选择逻辑散落在组件里；这样后续再补“下一条未读”或键盘导航时更容易复用。
- 阅读列表排序也适合先做前端层能力，只要底层保留服务端返回的完整集合，`Newest / Oldest / Unread First` 就能明显改善远程阅读效率，而不需要额外改 API。
